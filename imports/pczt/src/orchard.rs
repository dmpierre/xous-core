...
impl Spend {
    /// Convenience accessor that returns the alpha randomizer decoded as a
    /// `pallas::Scalar`, ready to pass into orchard's signing primitives
    /// (e.g. `SpendAuthorizingKey::randomize`). Returns `None` if alpha is
    /// unset or the bytes are not a canonical scalar (a malformed PCZT —
    /// the Constructor is supposed to write a canonical encoding via
    /// `pallas::Scalar::to_repr`).
    ///
    /// Provided so firmware-side low-level signers can fetch the scalar
    /// without depending on `pasta_curves` / `ff` directly; pczt already
    /// pulls them in under the `orchard` feature.
    pub fn alpha_scalar(&self) -> Option<pasta_curves::pallas::Scalar> {
        use ff::PrimeField;
        self.alpha
            .and_then(|bytes| pasta_curves::pallas::Scalar::from_repr(bytes).into())
    }
}
...
impl Bundle {
    /// Sets the spend authorisation signature on the action at `index`.
    ///
    /// No-op if `index` is out of range. Idempotent on identical inputs;
    /// re-setting overwrites a previously-stored signature.
    ///
    /// This is intended for low-level signers on memory-constrained targets
    /// that want to install a Signer-computed signature without going through
    /// [`Bundle::into_parsed`] + [`Bundle::serialize_from`]. That round-trip
    /// allocates a fresh `Vec<u8>` per action's `enc_ciphertext` /
    /// `out_ciphertext`, which on Xous + RV32 has been observed to land in
    /// memory that overlaps the parsed action's struct, producing structured
    /// corruption inside the new `enc_ciphertext` buffer (cf.
    /// `zcashapp_multi_recv_corruption.md`). Setting the signature directly
    /// in the wire-format struct skips that allocation pattern entirely.
    pub fn set_spend_auth_sig(&mut self, index: usize, sig: [u8; 64]) {
        if let Some(action) = self.actions.get_mut(index) {
            action.spend.spend_auth_sig = Some(sig);
        }
    }
