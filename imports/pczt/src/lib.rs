...
impl Pczt {
    /// Mutable access to the Orchard bundle.
    ///
    /// Companion to the `orchard()` getter; needed by low-level signers that
    /// want to write fields directly into the wire-format struct (e.g. via
    /// [`crate::orchard::Bundle::set_spend_auth_sig`]) without going through
    /// `into_parsed → serialize_from`. See the comment on
    /// `set_spend_auth_sig` for why bypassing that round-trip matters on
    /// memory-constrained targets.
    #[cfg(feature = "orchard")]
    pub fn orchard_mut(&mut self) -> &mut orchard::Bundle {
        &mut self.orchard
    }
