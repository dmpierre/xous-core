...
pub struct App {
    ...
    /// Defaults inherited from the `zao ui` invocation; slash commands
...
impl App {
    pub fn new(datadir: Option<String>, port: Option<String>) -> Result<Self> {
        let mut input = TextArea::default();
        input.set_cursor_line_style(ratatui::style::Style::default());
        input.set_placeholder_text("Type a command, or '/' for the menu");

        let history_path = dirs_next::data_local_dir()
            .map(|d| d.join("zao").join("tui_history"))
            .or_else(|| dirs_next::home_dir().map(|h| h.join(".zao").join("tui_history")));
    ...
    fn welcome(&mut self) {
        self.transcript.push(TranscriptLine::Note(format!(
            "zao {} — type '/' for commands, Ctrl-D to exit",
            env!("ZAO_VERSION")
        )));
    }
    ...
    fn submit(&mut self) {
        ...
        if let Some(stripped) = text.strip_prefix('/') {
            ...
            // and remind them.
            self.transcript.push(TranscriptLine::Note(
                "Hint: prefix commands with '/'. E.g. '/wallet balance'".to_string(),
            ));
        }
    }
    ...
    /// `pczt propose` while they're typing flag values).
    ...
    fn show_tui_help(&mut self) {
        let lines = [
            ...
            "  /wallet <Tab>  show wallet subcommands",
            ...
            "All other commands match the CLI subcommands shown by 'zao --help'.",
        ];
        for line in lines {
            self.transcript.push(TranscriptLine::Note(line.to_string()));
        }
