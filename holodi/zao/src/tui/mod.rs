//! `zao ui` — chat-style command palette for the Baochip-1x Zcash wallet.
//!
...
//! exposes, so adding `zao foo` automatically shows up as `/foo`.
//!
...
//! long-running commands (sync, send) stream output line-by-line.
...
pub fn run(datadir: Option<&str>, port: Option<&str>) -> Result<()> {
    install_panic_hook();
    ...
    let mut app = App::new(datadir.map(String::from), port.map(String::from))?;
