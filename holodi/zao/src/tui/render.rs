...
/// Zcash brand yellow accent.
const ACCENT: Color = Color::Rgb(245, 184, 0);
...
fn draw_transcript(frame: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    // honest. Long tracing lines, `/guide`, and other multi-paragraph
    // output render cleanly; horizontal scroll for over-wide lines is
    ...
    let para = Paragraph::new(lines)
        .block(
            Block::default()
                ...
                .title(Span::styled(" zao ", Style::default().fg(ACCENT).add_modifier(Modifier::BOLD))),
        )
        .scroll((scroll, 0));
...
fn draw_status(frame: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    if let Some(datadir) = &app.default_datadir {
        spans.push(Span::raw("  •  "));
        spans.push(Span::styled(
