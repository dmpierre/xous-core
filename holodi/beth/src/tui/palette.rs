...
//!     `/send` all show command-name suggestions.
...
//!     between a `--rpc-url <URL>` pair). Caller hides the menu.
...
pub fn entries_for(input: &str) -> Vec<PaletteEntry> {
    ...
    // the menu (we don't want to suggest `--rpc-url` if the user
    // already typed `--rpc-url https://…`).
...
/// slash-form of the already-typed path (`""` at root, `"/wallet"` etc.).
fn subcmd_entries(
    cmd: &clap::Command,
    path_prefix: &str,
    prefix: &str,
...
/// `path_prefix` is the slash-form already typed (e.g. `"/send-token"`).
fn flag_entries(
    cmd: &clap::Command,
    path_prefix: &str,
    prefix: &str,
    ...
    for arg in cmd.get_arguments() {
        ...
        // Most beth flags take a value (long takes_value); only a few
